...
void TrainView::draw()
{
	...
	if (gladLoadGL())
	{
		...
		if (!this->backgroundShader)
			...
			Shader(
				"../WaterSurface/src/shaders/background.vert",
				nullptr, nullptr, nullptr,
				"../WaterSurface/src/shaders/background.frag");
		...
		if (!this->skybox)
		{
			...
			this->skybox = new TextureCube(paths);
		}
	...
	glm::mat4 model = glm::translate(glm::vec3(0, 100*sin(this->m_pTrack->trainU), 100*cos(this->m_pTrack->trainU)));	
	this->drawCube(model, this->tw->arcLength->value()); 
...
void TrainView::drawCube(glm::mat4 model, bool useTexture)
{
	this->texture->bind(0);
	glUniform1i(glGetUniformLocation(this->shader->Program, "u_texture"), 0);

	glUniform1i(glGetUniformLocation(this->shader->Program, "u_useTexture"), useTexture);
	...
	model_matrix = glm::scale(model_matrix, glm::vec3(100.0f, 100.0f, 100.0f));
	model_matrix = glm::rotate(model_matrix, glm::radians(-90.0f), glm::vec3(0.0, 1.0, 0.0));
	...
	model_matrix = glm::translate(model_matrix, glm::vec3(0, 0, -100));
	model_matrix = glm::scale(model_matrix, glm::vec3(100.0f, 100.0f, 100.0f));
	model_matrix = glm::rotate(model_matrix, glm::radians(180.0f), glm::vec3(0.0, 1.0, 0.0));
	model_matrix = glm::rotate(model_matrix, glm::radians(90.0f), glm::vec3(1.0, 0.0, 0.0));
...
void TrainView::drawBackground()
{
	glDepthMask(false);

	this->skybox->bind(10);
	
	glUniform1i(glGetUniformLocation(this->backgroundShader->Program, "u_skybox"), 10);

	glm::mat4 model_matrix = glm::mat4();
	model_matrix = glm::translate(model_matrix, glm::vec3(0, 1, 0));
	glUniformMatrix4fv(glGetUniformLocation(this->backgroundShader->Program, "u_model"), 1, GL_FALSE, &model_matrix[0][0]);
	this->aPlane.draw();
...
void plane::draw()
{
	...
	//bind VAO
	glBindVertexArray(this->instance->vao);
	glDrawElements(GL_TRIANGLES, this->instance->element_amount, GL_UNSIGNED_INT, 0);
...
void plane::generateVAO()
{
	...
	this->instance = new VAO;
	this->instance->element_amount = element.size();

	glGenVertexArrays(1, &this->instance->vao);
	glGenBuffers(4, &this->instance->vbo[0]);
	glGenBuffers(1, &this->instance->ebo);

	glBindVertexArray(this->instance->vao);

	// Position attribute
	glBindBuffer(GL_ARRAY_BUFFER, this->instance->vbo[0]);
	...
	// Color attribute
	glBindBuffer(GL_ARRAY_BUFFER, this->instance->vbo[3]);
	glBufferData(GL_ARRAY_BUFFER, color.size() * sizeof(GLfloat), color.data(), GL_STATIC_DRAW);
	glVertexAttribPointer(3, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(GLfloat), (GLvoid*)0);
	glEnableVertexAttribArray(3);

	//Element attribute
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, this->instance->ebo);
...
void waterPlane::generateVAO()
{
	...
	this->instance->element_amount = element.size();
	glGenVertexArrays(1, &this->instance->vao);
	glGenBuffers(3, &this->instance->vbo[0]);
	glGenBuffers(1, &this->instance->ebo);
